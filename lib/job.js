...
Job.prototype.config = function(config){
    ...
    fs.mkdirSync(this.config.workingDir + "/" + this.jobID);
    ...
    var github = new GitHubClient({
        ...
        ,pathPrefix: this.config.params.targetHost !== "github.com" ? "/api/v3" : ""
...
Job.prototype.cleanse = function()
{
    ...
    if(logContent.config.params.userPAT)
    {
        logContent.config.params.userPAT="--redacted-->";
    }
    return logContent;
};
...
Job.prototype.flushToFile = function () {
    ...
    if(this.config.deleteTempDir && this.config.deleteTempDir === true)
    {
